...
This page was created to reproduce some of the results and provide the code in **_R_ programming** used in the paper: 
...
>**Note**: The results from the generative deep modelling were not good due to many reasons explained in the paper. 
...
We extracted street images from the Mapillary database. Then performed a manual filtering process, removing images of low quality. The database after filtered can be found [here](Database/Images/). To avoid overfitting, we performed image augmentation to the filtered images. 
...
a) Download the [original dataset](Dataset/Images/) used for [Training](Dataset/Images/Train_filtered/) and [Test](Dataset/Images/Test_filtered/) after manually filtered, without augmentation.

b) Download the **features** extracted from object detection in two formats for inputs in the Conditional Variational Autoencoder: 
