...
#files_train_labels <- list.files("Train/Labels_Class",  full.names = TRUE, pattern = ".txt", all.files = TRUE)
labels_train <- read_excel('Train/Labels_Dummy/Labels_Dummy.xlsx')

files_test <- list.files("Test/Images",  full.names = TRUE, pattern = ".jpg", all.files = TRUE)
#files_test_labels <- list.files("Test/Labels_Class",  full.names = TRUE, pattern = ".txt", all.files = TRUE)
labels_test <- read_excel('Test/Labels_Dummy/Labels_Dummy_test.xlsx')
...
train_array <- array(unlist(Results_train), dim=c(114,240,320,3))
dim(train_array)
...
test_array <- array(unlist(Results_test), dim=c(45,240,320,3))
